...
public class JRecorder implements ScreenRecorderListener {
	...
	public void recordingStopped() {
		...
		if (!shuttingDown) {
			...
			dialog.setFilterNames(new String[] { "Above Capture Files" });
			dialog.setFilterExtensions(new String[] { "*.acap" });

			dialog.setFileName("MyCapture.acap");
			String saveFileName = dialog.open();

			// Save audio and video
			if (saveFileName != null) {
				File video = new File(saveFileName);
				File audio = new File(saveFileName.replaceAll(".acap", ".wav"));
				
				if (video != null) {

					if (!video.getName().endsWith(".acap"))
						video = new File(video + ".acap");
